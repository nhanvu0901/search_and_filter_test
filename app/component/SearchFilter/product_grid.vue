...
export default {
    ...
    methods: {
        ...
        editListProductVariable(outer_html, list_attribute, product, count) {
            ...
            container_element.classList.remove("scroll-trigger--offscreen");
            container_element.style = "--animation-order: " + count
            ...
            for (let i = 0; i < image_list.length; i++) {
                image_list[i].setAttribute('data-src-master', product.img_src)
                image_list[i].setAttribute('data-src-master', product.img_src);
                image_list[i].setAttribute('srcset', product.img_src + "&amp;width=165 165w," + product.img_src + "&amp;width=360 360w," + product.img_src + " 466w")
                image_list[i].setAttribute('src',product.img_src + "&amp;width=533")
                image_list[i].setAttribute('data-rimg-template', product.img_src + "&amp;width=165 165w," + product.img_src + "&amp;width=360 360w," + product.img_src + " 466w")
                image_list[i].alt = product.title
                image_list[i].setAttribute('data-srcset', product.img_src + "&amp;width=165 165w," + product.img_src + "&amp;width=360 360w," + product.img_src + " 466w")
                image_list[i].setAttribute('data-src', product.img_src + "&amp;width=165 165w," + product.img_src + "&amp;width=360 360w," + product.img_src + " 466w")
            }
            //fix the list_attribute

            for (let i = 0; i < list_attribute.length; i++) {
                let parentElement = doc.querySelectorAll(list_attribute[i].parent.type + "." + list_attribute[i].parent.attribute[0].class.trim().split(/\s+/).join('.'));
                for (let j = 0; j < parentElement.length; j++) {
                    // if (parentElement[j].children.length === 1) {
                    let childElement = null
                    if (list_attribute[i].child.attribute.length > 0) {
                        childElement = parentElement[j].querySelector(list_attribute[i].child.type + "." + list_attribute[i].child.attribute[0].class.trim().split(/\s+/).join('.'));
                    }
                    else{
                        childElement = parentElement[j].querySelector(list_attribute[i].child.type);
                    }
        ...
        generateGrid() {
            ...
            for (let item in this.product_list) {
                let new_edit_outterHTML = this.editListProductVariable(data_theme.child_class, JSON.parse(data_theme.list_attribute), this.product_list[item], count)
